...
java -jar -Dconfig.file="$REPO_ROOT"/conf/spaceturtle.conf -Djava.security.auth.login.config="$REPO_ROOT"/conf/jaas.conf -Djava.security.krb5.conf="$REPO_ROOT"/conf/krb5.conf "$REPO_ROOT"/build/SpaceTurtle.jar
